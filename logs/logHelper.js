...
var os = require("os");
// 加载配置文件  
var objConfig = JSON.parse(fs.readFileSync("./logs/log4js.json", "utf8"));

// 检查配置文件所需的目录是否存在，不存在时创建  
if (objConfig.appenders) {
    var baseDir = objConfig["customBaseDir"];
    os.platform()=="win32"&&(baseDir="C:"+baseDir);
    ...
    if (!fs.existsSync(dirpath)) {
        ...
        dirpath.split("/").forEach(function(dirname) {
            ...
            if (pathtmp) {
                ...
                //pathtmp = dirname;
                os.platform()!="win32"?(pathtmp="/"+dirname):(pathtmp = dirname);
            }
            console.log("C:"+pathtmp);
